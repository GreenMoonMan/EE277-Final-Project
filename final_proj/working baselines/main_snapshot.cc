...
{
    xil_printf("\r\n===== SNAPSHOT MODE =====\r\n");
    xil_printf("  a) Capture current frame to DDR\r\n");
    xil_printf("  b) Display saved snapshot (pause live)\r\n");
    ...
    u8* snapshotBuf = reinterpret_cast<u8*>(SNAPSHOT_ADDR);
    ...
    while (1)
    {
        ...
        {
        case 'a':
            xil_printf("Capturing current frame...\r\n");
            Xil_DCacheInvalidateRange((INTPTR)liveFrame, bytes);
            memcpy(snapshotBuf, liveFrame, bytes);
            Xil_DCacheFlushRange((INTPTR)snapshotBuf, bytes);
            snapshotTaken = true;
            xil_printf("Snapshot saved at DDR addr 0x%08X\r\n", (u32)snapshotBuf);
            break;

        case 'b':
        {
            if (!snapshotTaken) {
                xil_printf("No snapshot available. Press 'a' first.\r\n");
                break;
            }

            xil_printf("Displaying saved snapshot...\r\n");
            vdma_driver.resetWrite();
            vdma_driver.resetRead();
            ...
            readCfg.FrameStoreStartAddr[0] = (u32)snapshotBuf;

            // << wrapped vdma pointer in its own case scope >>
            ...
            xil_printf("Snapshot displayed on HDMI.\r\n");
            break;
        }
