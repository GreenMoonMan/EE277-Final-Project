...
class KeyEntry
{
	public:
		...
		KeyEntry(PmodKYPD* kypd, std::function<void(char*)> accept, std::function<void(char*)> deny);
		void setCode(char* code);
		void poll();
		const char* getCurrentCode() const { return _currCode; }


	private:
		// accept/deny functions
		std::function<void(char*)> acceptFunc;
		std::function<void(char*)> denyFunc;
